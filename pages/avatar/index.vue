<template>
  <fui-page class="fui-flex fui-flex-col" :addTips='false'>
    <view class="body">
      <view class="page-bg">
      ...
      <view class="fui-flex fui-flex-between fui-mt25 fui-ml15 fui-mr15">
        <view class="page-avatar-box">
          <view class="page-avatar" :class="canEdit && showEdit ? 'edit' : ''"
                @tap="canEdit && showEdit === false ? showEdit = true : ''">
        ...
        <view class="page-btns fui-flex-between fui-flex-col fui-pb5 fui-pt5">
          <button class="fui-fs14 fui-c-666 fui-fw400" @tap="auth">
            {{ isWeChatAvatar && vuex_user.status === 1 ? '重新获取' : '获取头像' }}
          </button>
          <button class="fui-fs14 fui-c-666 fui-fw400" v-show="isLogin && vuex_user.originAvatar"
                  @tap="getOriginAvatar">获取原头像
          </button>
      ...
      <view class="page-pendant-box fui-mt20 fui-m15 fui-p10">
        <!--        分类栏-->
        <scroll-view scroll-with-animation="true" :scroll-into-view="`type_${borderType}`" scroll-x="true"
                     class="page-pendant-table fui-flex fui-mb15" enable-flex="true">
          <view
              class="_table-item fui-mr10 _active"
              :id="`type_${item.type}`"
              v-for="(item, index) in borderCategory"
              :key="index">
        ...
        <scroll-view
            ...
            scroll-y="true"
            class="fui-flex scroll-x page-pendant-list"
            @scroll="onScroll"
            enable-flex="true">
          <view class="scroll-view-item fui-flex-center"
                v-for="(item, index) in borderList[borderType]"
                :key="item._id"
                @click="changeBorder(item)"
                :class="{'_active': borderId === item._id}">
    ...
    <!--      <button open-type="contact" :style="{color: color}" @contact="onContact" class="fui-fs12 fui-ml10 fui-fw200">-->
    <!--        联系客服-->
    <!--      </button>-->
    <!--    </view>-->
...
export default {
  ...
  methods: {
    ...
    async getBorderImage(on = 1) {
      ...
      // 点击切换菜单 回复初始状态
      const type = this.$data.borderType || 1
      if (on === 1) {
    ...
    chooseImage() {
      uni.chooseImage({
        ...
        success: (res) => {
          ...
          this.cutImage(res.tempFilePaths[0])
        }
      });
    },
    async secCheck(file) {
